            ...
            try {
                axios.get('http://167.172.183.142/api/user/getQattaByManager', {
                    ...
                    if (response.data.length != 0) {
                        ...
                        thisComponent.state.Qattas.length == 0 ?
                            setTimeout(() => {
                                alert(thisComponent.Language != 'EN' ? "ليس لديك قطط" : "There is no Qattas")
                            }, 100)
                            :
                            setTimeout(() => {
                                alert(thisComponent.Language != 'EN' ? "لا يوجد مزيد" : "End of results")
                ...
                <View style={{ flex: 1, width }} >
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1, paddingBottom: 70, paddingTop: 12 }} >
                        ...
                        <View style={[styles.row, { width, justifyContent: 'center', alignItems: 'center', marginTop: 16 }]} >
                            <View style={{ borderBottomColor: '#707070', borderBottomWidth: 2, height: 3, flex: 1, marginHorizontal: 18 }}></View>
                            <Text style={{ fontSize: 14, color: '#414141' }} >
                                {this.Language != "EN" ? 'مجموعات القطة' : 'Qatta Groups'}
                            </Text>
                            <View style={{ borderBottomColor: '#707070', borderBottomWidth: 2, height: 3, flex: 1, marginHorizontal: 18 }}></View>
                        </View>
                                ...
                                return (
                                    item.data ?
                                        <View key={index.toString()} style={[styles.row, { width, justifyContent: 'center', alignItems: 'center', marginTop: 16 }]} >
                                            <TouchableOpacity
                                                onPress={() => this.props.navigation.navigate('GroupInfo', { Qatta: this.props.User.isMember ? item.data.qattaID : item.data, qattaMemberID: item.data._id })}
                    ...
                    </ScrollView>
                    {
                        ...
                        //     <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, width, height: 65, backgroundColor: '#FFF', borderTopLeftRadius: 38, borderTopRightRadius: 38 }} >
...
// redux
export default connect(mapStateToProps, {})(Qatta)
