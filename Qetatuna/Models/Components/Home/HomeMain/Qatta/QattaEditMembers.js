...
class QattaEditMembers extends Component {
    constructor(props) {
        super(props);
        this.state = {
            _id: this.props.navigation.getParam('Qatta')._id,
            alarm: this.props.navigation.getParam('Qatta').alarm,
            ...
            worth: this.props.navigation.getParam('Qatta').worth,
            Members: this.props.navigation.getParam('Qatta').Members,
            isVisible: false,
            isVisible2: false,
            isVisible3: false,
            code: '966',
    ...
    saveMembers(bool) {
        this.setState({ isVisible3: true })
        if (bool) {
            // this.setState({ perMember: (parseFloat(this.state.totalValue) / this.state.Members.length).toFixed(2).toString() })
            // alert(this.state.perMember)
            const { _id, alarm, group, totalValue, perMember, worth, Members } = this.state
            const Qatta = {
                _id, alarm, group, totalValue,
                perMember: (parseFloat(totalValue) / Members.length).toFixed(2).toString(),
                worth, Members
            }
            this.props.navigation.navigate('QattaEdit', { Qatta: Qatta });
        } else {
                ...
                >
                    ...
                    <View style={[styles.row, { justifyContent: 'center', alignItems: 'center', marginVertical: 8 }]}>
                        ...
                        <Text style={{ color: '#017ED4', fontSize: 18, fontWeight: 'bold', marginHorizontal: 8, }}>
                            {this.Language != "EN" ? "أضافة عضو" : "Add members"}
                        </Text>
                        <View
                            style={{
                                borderBottomColor: '#000',
            ...
            >
                ...
                >
                    ...
                    <View style={[this.Language != "EN" ? styles.row : styles.rowReverse, { justifyContent: 'space-evenly', marginVertical: 8 }]}>
                        <TouchableOpacity onPress={() => this.setState({ isVisible2: false })} style={[styles.shadow, { width: '40%', backgroundColor: "#DBEDFA", borderRadius: 12, borderWidth: 4, paddingVertical: 8, borderColor: "#017ED4", justifyContent: 'center', alignItems: 'center' }]} >
                            <Text style={{ color: '#000', fontSize: 16, fontWeight: 'bold' }}>
                                {this.Language != "EN" ? "الغاء" : "Cancel"}
                            </Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => this.deleteMember()} style={[styles.shadow, { width: '40%', backgroundColor: "#DBEDFA", borderRadius: 12, borderWidth: 4, paddingVertical: 8, borderColor: "#017ED4", justifyContent: 'center', alignItems: 'center' }]} >
                            <Text style={{ color: '#000', fontSize: 16, fontWeight: 'bold' }}>
    ...
    renderOverlay3 = () => {
        return (
            <Overlay
                ...
                overlayBackgroundColor="#FFF"
                width={width - 18 * 2}
                height={this.state.overlayHeight3 + 45}
                borderRadius={34}
                overlayStyle={{ overflow: 'hidden' }}
            ...
            >
                <View
                    onLayout={(event) => {
                        var { x, y, width, height } = event.nativeEvent.layout;
                        this.setState({ overlayHeight3: height })
                    }}
                    style={[styles.column, { backgroundColor: '#FFF', justifyContent: 'center', paddingHorizontal: 24, paddingVertical: 40 }]}
                >


                    <Text onPress={() => this.setState({ isVisible3: false })} style={{ color: '#000', fontSize: 22, fontWeight: 'bold', position: 'absolute', left: 10, top: 10 }}>
                        {"×"}
                    </Text>
                    ...
                    <View style={[styles.row, { justifyContent: 'center', alignItems: 'center', marginVertical: 8 }]}>
                        ...
                        />
                        <Text style={{ color: '#017ED4', fontSize: 18, fontWeight: 'bold', marginHorizontal: 8, }}>
                            {this.Language != "EN" ? "أعادة التوزيع" : "Redistribution"}
                        </Text>
                        <View
                            style={{
                                borderBottomColor: '#000',
                    ...
                    <View style={[styles.row, { width: '100%', justifyContent: 'center', alignItems: 'center', marginVertical: 38 }]}>
                        <Text style={{ textAlign: 'center' }} >{this.Language != "EN" ? "هل تريد أعادة توزيع المبالغ الأجمالية لهذا الشهر بعد تعديل الاعضاء   " : "Do you want to redistribute the total sums for this month after modifying members?"}</Text>
                    </View>

                    <View style={[this.Language != "EN" ? styles.row : styles.rowReverse, { justifyContent: 'space-evenly', marginVertical: 8 }]}>
                        <TouchableOpacity onPress={() => this.saveMembers(false)} style={[styles.shadow, { width: '40%', backgroundColor: "#DBEDFA", borderRadius: 12, borderWidth: 4, paddingVertical: 8, borderColor: "#017ED4", justifyContent: 'center', alignItems: 'center' }]} >
                            <Text style={{ color: '#000', fontSize: 16, fontWeight: 'bold' }}>
                                {this.Language != "EN" ? "لا" : "No"}
                            </Text>
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => this.saveMembers(true)} style={[styles.shadow, { width: '40%', backgroundColor: "#DBEDFA", borderRadius: 12, borderWidth: 4, paddingVertical: 8, borderColor: "#017ED4", justifyContent: 'center', alignItems: 'center' }]} >
                            <Text style={{ color: '#000', fontSize: 16, fontWeight: 'bold' }}>
            ...
            <SafeAreaView style={styles.container} >
                ...
                <View style={[styles.column, { flex: 1, width }]} >
                    ...
                    <View style={[styles.row, { width, justifyContent: 'center', alignItems: 'center', marginVertical: 16 }]} >
                        ...
                        <Text style={{ fontSize: 18, color: '#0C546A', fontWeight: 'bold' }} >
                            {this.Language != "EN" ? '‫الأعضاء‬' : 'Members'}
                        </Text>
                        <View style={{ borderBottomColor: '#000', borderBottomWidth: 1, height: 2, flex: 1, marginHorizontal: 18 }}></View>
                    </View>
                        ...
                        <View style={[styles.shadow, { backgroundColor: '#FFF', height: '100%', width: '90%' }]} >
                            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1 }} >
                                ...
                                {/* <View style={[this.Language != "EN" ? styles.rowReverse : styles.row, { width: '100%', alignItems: 'center', padding: 10 }]} >
                                    ...
                                    <Item style={[styles.shadow, { elevation: 2, flex: 1, backgroundColor: '#FFF', height: 45, justifyContent: 'center', alignItems: 'center', borderBottomColor: '#fff', borderRadius: 12 }]}>
                                        <Input
                                ...
                                <View style={[styles.rowReverse, { flex: 1, marginVertical: 4, flexWrap: 'wrap', justifyContent: 'space-evenly', alignItems: 'flex-start' }]}>

                                    {
                                        this.state.Members.map((item, index) => {
                                            return (
                                                item.mobile == this.props.User.mobile ? null :
                                                    <View key={index.toString()} style={[styles.column, { width: "30%", aspectRatio: 0.7, backgroundColor: '#FFF', justifyContent: 'flex-start', alignItems: 'center' }]} >
                                                        ...
                                                        <View style={[styles.column, { alignItems: 'center', marginTop: 8 }]} >
                                                            <Text style={{ color: '#707070', fontWeight: 'bold' }} >{item.fullname}</Text>
                                                            {/* <Text style={{ color: '#017ED4', }} >{"تم الدفع"}</Text> */}
                                                        </View>
                                                    </View>
                ...
                <View style={{ width, height: 80 }} >
                    <View style={[this.Language != "EN" ? styles.rowReverse : styles.row, { flex: 1, justifyContent: 'space-evenly', alignItems: 'center' }]} >
                        ...
                        <TouchableOpacity onPress={() => this.setState({ isVisible2: true })} style={[this.Language != "EN" ? styles.row : styles.rowReverse, { width: "45%", backgroundColor: '#DBEDFA', height: 40, justifyContent: 'space-evenly', paddingVertical: 8, borderRadius: 12, borderColor: '#0069B1', borderWidth: 3, paddingHorizontal: 12 }]}>
                            <FontAwesome name={'trash-o'} style={{ fontSize: 16, color: '#0069B1' }} />
                ...
                <View style={{ width, height: 80, backgroundColor: '#017ED4', borderTopLeftRadius: 38, borderTopRightRadius: 38 }} >
                    <View style={[styles.row, { flex: 1, justifyContent: 'center', alignItems: 'center' }]} >
                        <TouchableOpacity onPress={() => this.save()} style={[styles.row, { width: width * 0.5, justifyContent: 'center', backgroundColor: '#FFF', paddingVertical: 8, borderRadius: 16, borderColor: '#0069B1', borderWidth: 1, paddingHorizontal: 12 }]}>
                            <Text style={{ color: '#000', textAlign: 'center', fontSize: 12, fontWeight: 'bold' }} >{this.Language != "EN" ? "احفظ الاعضاء" : "Save members"}</Text>
                        </TouchableOpacity>
                    </View>
                </View>
...
// redux
export default connect(mapStateToProps, {})(QattaEditMembers)
