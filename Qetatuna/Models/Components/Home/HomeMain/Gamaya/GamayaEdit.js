...
class GamayEdit extends Component {
    constructor(props) {
        super(props);
        this.state = {
            Alarms: [],
            Processing: false,
            _id: this.props.navigation.getParam('Gamaya') ? this.props.navigation.getParam('Gamaya').data._id : '',
            group: this.props.navigation.getParam('Gamaya') ? this.props.navigation.getParam('Gamaya').data.group : '',
            aljameiaType: this.props.navigation.getParam('Gamaya') ? this.props.navigation.getParam('Gamaya').data.aljameiaType : 1,
            aljameiaAmount: this.props.navigation.getParam('Gamaya') ? this.props.navigation.getParam('Gamaya').data.aljameiaAmount : '',
            membersNum: this.props.navigation.getParam('Gamaya') ? this.props.navigation.getParam('Gamaya').data.membersNum : '',
            startDate: this.props.navigation.getParam('Gamaya') ? this.props.navigation.getParam('Gamaya').data.startDate : new Date(), //this.props.User.birthday,
            alarm: this.props.navigation.getParam('Gamaya') ? this.props.navigation.getParam('Gamaya').data.alarm : "0",
            members: this.props.navigation.getParam('Gamaya').membersNumber ? this.props.navigation.getParam('Gamaya').membersNumber : this.props.navigation.getParam('Gamaya').members,
        };
    }
    ...
    handleBackButtonClick = () => {
        this.props.navigation.navigate('GamayaMain', { Gamaya: this.props.navigation.getParam('Gamaya') });
        return true;
    }
            ...
            [
                ...
                },
                {
                    text: this.props.Language != "EN" ? "نعم" : 'OK', onPress: () => {
                        ...
                        try {
                            axios.put('http://167.172.183.142/api/user/deletealjameia/' + id, {
    ...
    renderHeader() {
        return (
            <View style={[styles.flex, styles.row, styles.shadow, { width, height: 50, alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#F6F6F6' }]} >
                ...
                <Text style={{ color: '#000', fontSize: 16 }}>{this.Language != "EN" ? "تعديل الجميعيه" : "Edit Gamaya"}</Text>
    ...
    SearchAndGetObjectFromArray(arr, id) {
        // console.log( arr, id)
        return arr.find(obj => obj._id == id._id)
    }
    ...
    addMembers() {
        ...
        if (this.state.membersNum == '' || parseInt(this.state.membersNum) < 2) {
            ...
            if (this.state.aljameiaAmount == '' || parseInt(this.state.aljameiaAmount) < 2) {
                ...
                this.props.navigation.navigate('GamayaEditMembers', {
                    Gamaya: {
                        _id: this.state._id,
            ...
            <SafeAreaView style={styles.container} >
                ...
                <View style={{ flex: 1, width }}>
                    ...
                    <View style={[styles.row, { width, justifyContent: 'center', alignItems: 'center', marginVertical: height * 0.025 }]} >
                        ...
                        <Text style={{ fontSize: 18, color: '#000' }} >
                            {this.Language != "EN" ? 'بيانات الجمعية' : "Gamaya Details"}
                        </Text>
                        <View style={{ borderBottomColor: '#000', borderBottomWidth: 1, height: 2, flex: 1, marginHorizontal: 18 }}></View>
                    </View>
                    ...
                    <View style={[styles.row, { width, flex: 1, justifyContent: 'center', alignItems: 'center', marginVertical: height * 0.025 }]} >
                        <View style={[styles.shadow, { backgroundColor: '#FFF', width: '90%', height: '100%' }]} >
                                    ...
                                    <View style={[styles.rowReverse, { width: '70%', backgroundColor: '#FFF', alignItems: 'center', justifyContent: 'center' }]}>
                                        <View style={[styles.rowReverse, { width: '100%', backgroundColor: '#FFF', alignItems: 'center', paddingHorizontal: 8, justifyContent: 'center' }]}>
                                            <TouchableOpacity onPress={() => this.addMembers()} style={[styles.shadow, { elevation: 2, width: '100%', height: height * 0.06, minHeight: 35, maxHeight: 60, backgroundColor: '#FFF', borderRadius: 8, borderColor: "#E9E9E9", borderWidth: 2, justifyContent: 'center', alignItems: 'center', overflow: 'hidden' }]} >
                                                <Text style={{ color: this.state.group != '' ? '#000' : '#707070' }} >{this.state.group != '' ? this.state.group : this.Language != "EN" ? 'ضف المجموعه' : "Add group"}</Text>
                                            </TouchableOpacity>
                                        </View>
                                    </View>
                                </View>
                                    ...
                                    <View style={[styles.rowReverse, { width: '70%', backgroundColor: '#FFF', alignItems: 'center', paddingHorizontal: 8, justifyContent: 'center' }]}>
                                        {
                                            ...
                                            //     <Picker
                                            //         selectedValue={this.state.membersNum ? this.state.membersNum.toString() : "2"}
                                            //         mode="dropdown"
                ...
                <View style={{ width, height: 80, backgroundColor: '#E1D115', borderTopLeftRadius: 38, borderTopRightRadius: 38 }} >
                    ...
                    <View style={[styles.row, { flex: 1, justifyContent: 'center', alignItems: 'center' }]} >
                        <TouchableOpacity onPress={() => this.addMembers()} style={[styles.row, { width: width * 0.5, justifyContent: 'center', backgroundColor: '#FFF', paddingVertical: 8, borderRadius: 16, borderColor: '#CBBE21', borderWidth: 1, paddingHorizontal: 12 }]}>
...
// redux
export default connect(mapStateToProps, {})(GamayEdit)
