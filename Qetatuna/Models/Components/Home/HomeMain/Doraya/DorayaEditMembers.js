...
class DorayaEditMembers extends Component {
    constructor(props) {
        super(props);
        this.onDrop = this.onDrop.bind(this);
        this.onHover = this.onHover.bind(this);
        this.state = {
            group: this.props.navigation.getParam('DorayaEdited') ? this.props.navigation.getParam('DorayaEdited').group : '',
            dorayaRepeatType: this.props.navigation.getParam('DorayaEdited') ? this.props.navigation.getParam('DorayaEdited').dorayaRepeatType : 1,
            repeatsPerWeek: this.props.navigation.getParam('DorayaEdited') ? this.props.navigation.getParam('DorayaEdited').repeatsPerWeek : 1,
            date: this.props.navigation.getParam('DorayaEdited') ? this.props.navigation.getParam('DorayaEdited').date : new Date(),
            dorayaTime: this.props.navigation.getParam('DorayaEdited') ? this.props.navigation.getParam('DorayaEdited').dorayaTime : -1,
            members: this.props.navigation.getParam('DorayaEdited') ? this.props.navigation.getParam('DorayaEdited').members : [],
            code: '966',
            cca2: 'SA',
            fullname: '',
            mobile: '',
    ...
    handleBackButtonClick = () => {
        ...
        // const DorayaEdited = this.props.navigation.getParam("DorayaEdited")
        // this.props.navigation.navigate('DorayaEdit', { Doraya: Doraya, DorayaEdited: DorayaEdited });
        const Doraya = this.props.navigation.getParam("Doraya")
        const { group, dorayaRepeatType, repeatsPerWeek, date, dorayaTime, members } = this.state
        const DorayaEdited = { group, dorayaRepeatType, repeatsPerWeek, date, dorayaTime, members }
        this.props.navigation.navigate('DorayaEdit', { Doraya: Doraya, DorayaEdited: DorayaEdited });
        return true;
    }
    ...
    getMembers() {
        var newArr = []
        this.props.navigation.getParam('DorayaEdited').members.map((item, index) => {
            item.membersID ?
        ...
        return (
            <SafeAreaView style={styles.container} >
                ...
                <View style={{ width, height: 80 }} >
                    <View style={[this.Language != "EN" ? styles.rowReverse : styles.row, { flex: 1, justifyContent: 'space-evenly', alignItems: 'center' }]} >
                        <View style={[this.Language != "EN" ? styles.rowReverse : styles.row, { justifyContent: 'space-evenly', alignItems: 'center', width: '45%' }]} >
                            ...
                            <Text >{this.Language != "EN" ? "اضافه عضو جديد" : "Add member"} </Text>
                        </View>
                        <TouchableOpacity onPress={() => this.setState({ isVisible2: true })} style={[this.Language != "EN" ? styles.row : styles.rowReverse, { width: "45%", backgroundColor: '#DBF4FA', height: 40, justifyContent: 'space-evenly', paddingVertical: 8, borderRadius: 12, borderColor: '#00C4CA', borderWidth: 3, paddingHorizontal: 12 }]}>
                ...
                <View style={{ width, height: 80, backgroundColor: '#00C4CA', borderTopLeftRadius: 38, borderTopRightRadius: 38 }} >
                    <View style={[styles.row, { flex: 1, justifyContent: 'center', alignItems: 'center' }]} >
                        <TouchableOpacity onPress={() => this.save()} style={[styles.row, { width: width * 0.5, justifyContent: 'center', backgroundColor: '#FFF', paddingVertical: 8, borderRadius: 16, borderColor: '#00A5AA', borderWidth: 1, paddingHorizontal: 12 }]}>
                            <Text style={{ color: '#000', textAlign: 'center', fontSize: 12, fontWeight: 'bold' }} >{this.Language != "EN" ? "احفظ القائمه" : "Save list"}</Text>
                        </TouchableOpacity>
                    </View>
                </View>
...
class DraggyInner extends React.Component {
    ...
    render() {
        if (this.props.dragOver && !this.props.ghost) {
            // drag over
            return (
                <View style={[styles.rowReverse, { width: '100%', backgroundColor: 'transparent', alignItems: 'center', marginBottom: 12, paddingHorizontal: 18, marginTop: -5, justifyContent: 'center' }]}>
                    <View style={[styles.rowReverse, { width: '100%', height: 50, alignItems: 'center' }]} >
                        ...
                        <View style={{ flex: 1, height: 50, justifyContent: 'center', alignItems: 'center' }} >
                            <Text style={{ fontSize: 12, color: '#0C546A' }} >
                                {
                                    this.props.item.turnDate && this.props.item.turnDate != undefined && this.props.item.turnDate != null ?
                                        new Date(this.props.item.turnDate).getDate()
                                        + '-' +
                                        parseInt(new Date(this.props.item.turnDate).getMonth() + 1)
                                        + '-' +
                                        new Date(this.props.item.turnDate).getFullYear()
                                        :
                                        "--"
                                }
                            </Text>
...
class Draggy extends React.Component {
    render() {
        return (
            <Draggable data={this.props.item} style={[{ margin: 7.5 }, this.props.styles]}>
                <DropZone
...
// redux
export default connect(mapStateToProps, {})(DorayaEditMembers)
